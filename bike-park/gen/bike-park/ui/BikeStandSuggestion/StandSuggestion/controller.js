angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.config(["entityApiProvider", function (entityApiProvider) {
		entityApiProvider.baseUrl = "/services/ts/bike-park/gen/bike-park/api/BikeStandSuggestion/StandSuggestionService.ts";
	}])
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("StandSuggestion", `Unable to count StandSuggestion: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("StandSuggestion", `Unable to list/filter StandSuggestion: '${response.message}'`);
						return;
					}
		...
		$scope.deleteEntity = function () {
			...
			messageHub.showDialogAsync(
				'Delete StandSuggestion?',
				`Are you sure you want to delete StandSuggestion? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("StandSuggestion", `Unable to delete StandSuggestion: '${response.message}'`);
							return;
						}
						refreshData();
						$scope.loadPage($scope.dataPage, $scope.filter);
